...
plot.volcano = function(resultsObject, lfc_cutoff, alpha, showText=false) {
...
isEntrezID = function(v) {
  all(sapply(v, function(s) startsWith(s, 'ENSG')))
}
...
addAnno = function(res, species) {
  ...
  ens.str<-sapply(rownames(res), function(s) {t=strsplit(s,'\\.'); return(t[[1]][1])})
